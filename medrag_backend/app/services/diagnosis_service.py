...
class DiagnosisService:
    ...
    def _is_diagnosis_clear(candidates: List[Dict[str, Any]], round_index: int, max_rounds: int) -> bool:
        ...
        second_score = int(candidates[1].get("confidence", 0)) if len(candidates) > 1 else 0
        return top_score >= 78 and top_score - second_score >= 18

    @staticmethod
    def _support_score(value: Any, fallback: int) -> int:
        try:
    ...
    async def finalize(self, session_id: str, answers: List[FollowUpAnswer]) -> Dict[str, Any]:
        ...
        if refined and refined.get("diagnoses"):
            ...
            for idx, candidate in enumerate(raw_candidates[:3]):
                ...
                diagnoses.append(
                    {
                        "disease": llm_item.get("disease", candidate["disease"]),
                        "support_score": self._support_score(
                            llm_item.get("support_score"), candidate["confidence"]
                        ),
        ...
        return {
            "session_id": session.session_id,
            "diagnoses": diagnoses,
            "major_diagnosis": diagnoses[0]["disease"] if diagnoses else "待明确",
            "minor_diagnoses": [item["disease"] for item in diagnoses[1:3]],
            "knowledge_graph_hits": self.knowledge.build_graph_context(session.symptoms, typed_answers),
            ...
            "disclaimer": "本次辅助诊断仅用于参考，不能替代临床判断；具体诊疗方案请结合患者实际临床情况、体格检查、辅助检查结果及医生意见确定。如出现胸痛、呼吸困难、意识异常、严重出血、持续高热等急危重症请立即就医。",
        }
